...
export function VoucherPage() {
  ...
  const [buyPinError, setBuyPinError] = useState<string | null>(null);
  // Success modal
  ...
  const handleConfirmBuy = async () => {
    ...
    if (buyPin.length !== 6) {
      setBuyPinError("PIN harus tepat 6 digit angka");
      return;
    }
    setBuyLoading(true);
    setBuyError(null);
    ...
    try {
      const res = await authFetch(`${API_BASE_URL}/api/vouchers/buy`, {
        method: "POST",
        headers: authHeader(),
        body: JSON.stringify({ voucherId: buyVoucher.id, pin: buyPin }),
      });
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 sm:p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => { setBuyVoucher(null); setBuyPin(""); setBuyPinError(null); }}
            ...
            {/* PIN Input */}
            <div className="mb-5">
              <label className="font-['Poppins',sans-serif] font-semibold text-gray-700 text-sm block mb-1">
                PIN Kamu
              </label>
              <p className="font-['Lato',sans-serif] text-gray-400 text-xs mb-2">
                Masukkan PIN 6 digit sebagai konfirmasi pembelian
              </p>
              <div className="relative">
                ...
                <input
                  type={showBuyPin ? "text" : "password"}
                  value={buyPin}
                  onChange={(e) => {
                    setBuyPin(e.target.value.replace(/\D/g, "").slice(0, 6));
                    if (buyPinError) setBuyPinError(null);
                  }}
                  placeholder="6 digit angka"
                  inputMode="numeric"
                  maxLength={6}
                  className={`w-full pl-12 pr-12 py-3 border-2 ${buyPinError ? "border-red-500" : "border-gray-200"} rounded-xl font-['Lato',sans-serif] text-base text-gray-900 bg-white placeholder:text-gray-400 focus:border-bsi-teal-primary focus:outline-none transition-colors tracking-widest`}
                />
                <button
                  type="button"
                  onClick={() => setShowBuyPin(!showBuyPin)}
              ...
              {buyPinError && (
                <p className="mt-1 text-sm text-red-500 font-['Lato',sans-serif]">{buyPinError}</p>
              )}
            </div>
