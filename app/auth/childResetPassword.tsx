...
export function ChildResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setError("Token reset tidak ditemukan. Gunakan link dari email orang tua.");
      return;
    }
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => router.push("/auth/child/login"), 3000);
    } catch {
  ...
  if (!token) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-bsi-orange-primary to-bsi-orange-secondary flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md text-center">
          ...
          <p className="font-['Lato',sans-serif] text-gray-500 text-sm mb-6">
            Token reset tidak ditemukan. Pastikan membuka link langsung dari email orang tua.
          </p>
          <Link
