...
export function PendingActionsPage() {
  ...
  const [allChores, setAllChores] = useState<AllChore[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (choresData.success) {
        setAllChores(choresData.data);
        setChores(choresData.data.filter((c: Chore) => c.status === "PENDING_REVIEW"));
      }
  ...
  const approvedToday = allChores.filter(
    (c) => c.status === "APPROVED" && c.updatedAt && new Date(c.updatedAt).toDateString() === today
  ).length;
  const rejectedToday = allChores.filter(
    (c) => c.status === "REJECTED" && c.updatedAt && new Date(c.updatedAt).toDateString() === today
  ).length;
  const totalRewardsPending = chores.reduce((sum, c) => sum + c.rewardAmount, 0);
